...
# Implementation of a perceptron learning algorithm. This learning method is based on the MCP neuron model.
# Perceptron:     [1, x1, ..., xm] --> [w0, w1, ..., wm] --> z = w0 + x1w1 + ... + xmwm --> (threshold function) sgn{z} --> y: output & w update
...
class Perceptron(object):
    ...
    def net_input(self, X):
        # calculate net input (ypi) -> wT*x
        return np.dot(X, self.w_[1:]) + self.w_[0]

    def predict(self, X):
        # return class label after sign function, same as:
...
df = pandas.read_csv('iris.data', header=None, encoding='utf-8')
print(df.tail())
...
# plt.savefig('images/iris_plot.png', dpi=300)
plt.show()
